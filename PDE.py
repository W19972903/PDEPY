...
- An equation is a pair eq=(d,p), where d is a derivative (e.g. u00, u12, ...) and p is a polynomial.
- A system is a list of equations S=[eq1,...,eqk].
- A subsystem is a pair C=(S,Y), with S a system and Y a list of independent variables.
- A stratified system is a list of subsystems H=[C1,..,Cm]. Here C1 must be the only main subsystem.
...
2) CONSERVATION LAWS FOR A WAVE EQUATION IVP
...
NT,T=findCL(Hw,P0=[],base=[t,x,u0_0,u1_0,u0_1,v00],degree=2) # NT+T is a basis of all polynomial C.L.s (density-flux pairs) of degree <=2 in the given base of variables
...
NT,T=findCL(Hw,P0=[],base=[t,x,u0_0,u1_0,u0_1,v00],degree=2) 
...
def check0(base, newinst,Xpar):
    ...
    if type(base)==type([]):
        #print(base)
        G = groebner(base,Xpar,order='grevlex',domain='QQ')#order='grevlex',field=True)
    else:
...
def check01(base, newinst,Xpar):
    ...
    if Xpar==[]:
        return (newinst==0), base
    if type(base)==type([]):
        G=groebner(base,Xpar,order='grevlex',domain='QQ')#field=True)
    else:
        G = base # assume base is already a Groebner basis
    lg=len(G)
    for q in newinst:
...
def post(pt,Huser,P,Plist,Monotony=False,extraptlist=None):   
    ...
    global D, DI, NIND, HT, Xlist, H, zeropar,Xpar, zeroind
    H=ext2int(Huser) 
    pt=Poly(pt,Xlist)
    Xind=Xlist[:NIND]
    ...
    if qt==0:
        ...
        _,rt=reduced(Poly(qt,Plist+Xpar,domain='QQ'),G0)  # rt used to extract linear constraints for membership in JR
    border=[]
    print('*** Level m=0 ***')
    if Monotony:
    ...
    for lin in coeffs:      # solve the list of linear constraints, one at a time
    ...
    for j in range(maxiter):
        ...
        if qt==0:
            ...
            #print(qt,Xpar)
            _,rt=reduced(Poly(qt,Plist+Xpar,domain='QQ'),G0) # rt= S_H(newpt) mod G0
        coeffs = Poly(rt,Xpar).coeffs() # list of linear expressions (constraints) extracted as coefficients of rt      
        sigma=[]                                                
...
def findCLn(H,P0=[],base=None,ext=[],degree=2):
    global Xlist, NIND
    if base==None:
        newbase=[u0_0,u1_0,u0_1,u1_1,u2_0,u0_2]
    newbase=base+ext
    ...
    X=Xlist[:NIND]
    for x in X:
        pt,pl=genpt(newbase,degree,offset=offs)
        Dx=totalder(Poly(pt,Xlist),x)/1
        ptList.append(pt)
        #plList.append(pl)
        totalpl=totalpl+pl
        Dlist.append(Dx)
    ...
    #x=Xlist[1]
    sumpt=sum(Dlist)
    qt,J,lc=post(sumpt,H,P0,totalpl,extraptlist=ptList)
    start_timefreepar = time.time()
    print("   Starting calculation of free parameters left...")
    freepar=set([])
    for pt in lc:
        freepar= freepar.union(pt.free_symbols)
    freepar=freepar.difference(set(Xlist))
    #sigma0={a:0 for a in freepar}
    print("   Elapsed time(freepar): %s seconds ---" % (time.time() - start_timefreepar))
    start_timecc = time.time()
    print("   Starting construction of conserved currents...")    
    J=[]
    lc=[Poly(pt,freepar) for pt in lc]
    zeroarg=[0]*len(freepar)
    for j in range(len(freepar)):
        sigmaj=zeroarg.copy()
        sigmaj[j]=1
        J.append([pt(*sigmaj) for pt in lc])
    print("   Elapsed time(cc): %s seconds ---" % (time.time() - start_timecc))
    #print("--- Elapsed time: %s seconds ---" % (time.time() - start_time))
    print("   Filtering trivial laws...")
    with suppress_stdout():
        NT,T=checktrivialn(J,H,P0)
    return NT,T
    
def checktrivialn(J,H,P0):  
    global Xlist, NIND
    X=Xlist[:NIND]    
    NT=[]
    T=[]
    for cl in J:
        dxlist=[totalder(Poly(cl[i],Xlist),X[i]) for i in range(NIND)]
        Nabla=sum(dxlist)
        if Nabla==0:
            T.append(cl)
        else:
